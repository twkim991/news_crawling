...
from text_processing import build_text, ensure_schema
...
def load_ag_news(path):
    ...
    df["text"] = [build_text(title, description) for title, description in zip(df["title"], df["description"])]
...
def load_newsapi(path):
    ...
    df["text"] = [build_text(title, description, content) for title, description, content in zip(df["title"], df["description"], df["content"])]
