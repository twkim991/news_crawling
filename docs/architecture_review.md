...
3. **공통 모듈 책임 집중**
   - 전처리/임베딩/분류 로직이 한 곳에 몰리면 유지보수가 어려워집니다.
4. **스크립트 중심 실행 구조**
   - 수집/분석/보고 단계를 기능 모듈과 CLI로 더 분리할 필요가 있습니다.

## 5) 프로젝트 원칙
- 이 프로젝트는 정답셋 기반 평가 파이프라인 구축을 목표로 하지 않습니다.
- 핵심 목적은 다양한 뉴스 소스를 공통 구조로 정리하고 기술 스택 트렌드를 탐색 가능한 형태로 만드는 것입니다.

## 6) 다음 단계
- stack/entity alias 사전 고도화
- event type 분류 추가
- 공통 모듈 분리 및 설정 구조 정리
- GDELT/NewsAPI/SSAFY 실행 진입점 정리
