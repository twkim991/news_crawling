...
INPUT_PATH = os.path.join("data", "processed", "newsapi_processed.csv")
OUTPUT_DIR = "outputs"
...
def _apply_binary_tech_classifier(
    ...
    print("[NewsAPI Analysis] Load AG binary classifier")
    clf = joblib.load(model_path)

    print("[NewsAPI Analysis] Encode texts for binary classification")
    embeddings = encode_texts(text_signal.tolist(), batch_size=batch_size)

    if not isinstance(embeddings, np.ndarray):
        embeddings = np.asarray(embeddings)

    print("[NewsAPI Analysis] Predict tech / non-tech")
...
def _prepare_article_dates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if date_col is None:
        raise ValueError(
            "날짜 컬럼을 찾지 못했습니다. processed NewsAPI 파일에 published_at / publishedAt / date 중 하나는 있어야 합니다."
        )
...
def _finalize_classified_articles(df: pd.DataFrame) -> pd.DataFrame:
    ...
    working["primary_stack"] = np.where(
        ...
        np.where(
            working["tracked_stack_count"] > 0,
            working["tracked_stack_list"].str[0],
            "Unspecified",
        ),
        primary_stack,
    )
...
def _get_all_tracked_stacks() -> pd.DataFrame:
    ...
    return (
        pd.DataFrame(rows)
        .sort_values(["stack_category", "stack_subgroup", "stack_name"])
        .reset_index(drop=True)
    )
...
def run_newsapi_analysis(
    ...
    print("[NewsAPI Analysis] Filter empty text rows")
    ...
    if df.empty:
        print("[NewsAPI Analysis] No rows left after filtering. Abort.")
        return None

    print("[NewsAPI Analysis] Binary tech classification start")
    ...
    print("[NewsAPI Analysis] Binary tech classification done")

    print("[NewsAPI Analysis] Dev-tech keyword gate start")
    gated_df = _apply_devtech_keyword_gate(scored_df)
    print("[NewsAPI Analysis] Dev-tech keyword gate done")
    ...
    print("[NewsAPI Analysis] Rows kept for final classification:", len(tech_df))

    if tech_df.empty:
        print("[NewsAPI Analysis] No rows passed final gate. Abort.")
        return None
    ...
    print("[NewsAPI Analysis] Subcategory + stack classification start")
    result_df = classify_subcategory(tech_df)
    print("[NewsAPI Analysis] Subcategory + stack classification done")

    full_classified_df = _finalize_classified_articles(result_df)
    if full_classified_df.empty:
        print("[NewsAPI Analysis] No classified rows after date normalization.")
        return None
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Analyze processed NewsAPI tech data and build stack trend scores"
    )
    parser.add_argument("--year-month", default=None, help="month in YYYY-MM format")
    parser.add_argument("--input-path", default=INPUT_PATH, help="processed NewsAPI csv path")
    parser.add_argument("--output-dir", default=OUTPUT_DIR, help="directory to save analysis outputs")
    parser.add_argument("--output-prefix", default="newsapi", help="prefix for output file names")
    ...
    if args.year_month:
        ...
        input_path = args.input_path or os.path.join(
            "data",
            "processed",
            f"newsapi_processed_{suffix}.csv",
        )
        output_dir = args.output_dir or os.path.join("outputs", suffix)
        output_prefix = args.output_prefix or f"newsapi_{suffix}"
    else:
        input_path = args.input_path or INPUT_PATH
        output_dir = args.output_dir or OUTPUT_DIR
        output_prefix = args.output_prefix or "newsapi"

    os.makedirs(output_dir, exist_ok=True)

    print("[NewsAPI Analysis] input_path :", input_path)
    print("[NewsAPI Analysis] output_dir :", output_dir)
    print("[NewsAPI Analysis] output_prefix :", output_prefix)
